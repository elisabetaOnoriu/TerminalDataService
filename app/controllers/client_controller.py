...
def create_client(client: ClientCreate, db: Session = Depends(get_db)) -> ClientRead:
    """
...
def assign_device_to_client(
    ...
    try:
        ...
        db.rollback()
        logger.error("DB error while assigning device %s to client %s: %s", device_id, client_id, e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        db.rollback()
        logger.error("Unexpected error while assigning device %s \
        to client %s: %s", device_id, client_id, e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
